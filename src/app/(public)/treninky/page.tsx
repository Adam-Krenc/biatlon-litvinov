...
export default async function TreninkyPage() {
  const [page, session] = await Promise.all([
    prisma.staticPage.findUnique({ where: { slug: "treninky" } }),
    auth(),
  ]);
  if (!page) notFound();
    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      {isAdmin(session) && (
        <div className="mb-4 flex justify-end">
          <Link href="/admin/stranky/treninky" className="bg-[#1a3a6b] text-white px-3 py-1.5 rounded text-sm hover:bg-[#0f2448]">
            Upravit stránku
          </Link>
        </div>
      )}
